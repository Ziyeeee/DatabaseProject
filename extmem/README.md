...
（正常情况下，在二次扫描时应该从两个关系的每一个子集中读取一块进入缓冲区进行处理。但是，本题设计的缓冲区过小，若按正常读取则没有空间存储待写入数据。因此，改变循环的顺序，读取关系R中一个子集的一块数据和关系S四个子集的四块数据。该方法在停止策略下与原方法复杂度相同。）

工程文件见`experiment5`

### 6. 基于排序的交运算：

​		基于排序或散列的两趟扫描算法，实现其中一种集合操作算法：并、交、差（S-R）中的一种。将结果存放在磁盘上，并统计操作后的元组个数。
...
基于排序或散列的两趟扫描算法，实现剩余的集合操作算法。将结果存放在磁盘上，并统计操作后的元组个数。
...
7. 从关系C的四个子集中分别读取一块数据到缓冲区
8. 比较关系R和C的子集中的元组
9. 当属性值R.A与S.C不同时，分别保存两个元组；当属性值R.A与S.C相同时，仅保存一个元组
10. 转到步骤7直至读取完所有的关系C的子集中的数据块
11. 转到步骤6直至读取完所有的关系R的子集中的数据块
12. 显示磁盘存储地址及连接次数
...
​		提供的exemem.c文件在申请一个新的blk时未作初始化，在释放一个blk时也没有真正清除其中的数据。再重复申请、释放时会导致旧数据被错误的读入。因此，在最后我完善了exemem.c文件，在getNewBlockInBuffer函数中加入了初始化操作，清除原有的旧数据。此外，原始部分数据NULL的位置被错误的替换为空格，不过问题不大，不影响程序运行，因此也没有特意修正。